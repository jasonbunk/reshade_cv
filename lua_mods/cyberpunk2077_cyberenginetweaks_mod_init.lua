-- this function comes from other mods
...
registerForEvent("onUpdate", function(delta)
  if not IsPlayerInAnyMenu() then
    if MyCamCoordsStash.CamTransform then
      MyCamCoordsStash.counter = MyCamCoordsStash.counter + 1;
    else
      MyCamCoordsStash.CamTransform = Transform.new();
      MyCamCoordsStash.counter = 3;
    end
    if Game.GetCameraSystem():GetActiveCameraWorldTransform(MyCamCoordsStash.CamTransform) then
...
registerForEvent("onInit", function()
  -- The buffer is all float64
  MyCamCoordsStash.contiguousmembuf[1] = 1.38097189588312856e-12 -- magic value which rarely occurs in game memory, so can be efficiently scanned for
  for i=2,17 do
    MyCamCoordsStash.contiguousmembuf[i] = i
  end
end)
